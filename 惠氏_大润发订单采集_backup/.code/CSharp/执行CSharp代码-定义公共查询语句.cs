...
public void Run()
{
    ...
    curSoldToShipToSql = string.Format("select dc_name as DC, dc_no as DC编号, store_location as 门店, ship_to as 'Ship to', sold_to as 'Sold to', wyeth_dc_no as 惠氏大仓账号, region, customer_pay_method as 支付方式, dms_account DMS账号, discount from ship_to_sold_to where customer_name='{0}'", customer_name);
    constraintsSql = string.Format("SELECT sku_code, comment FROM constraint_list where ver=(select max(ver) from constraint_list)");
    specialSkuSql = string.Format("SELECT sold_to, sku_code, comment FROM special_products where customer_name like '{0}%'", customer_name);
    lastCaptureDateSql = string.Format("select max(create_date_time) maxCreatedTime from {0} where customer_name='{1}'", dtRow_ProjectSettings["订单数据库表名"].ToString(), customer_name);
