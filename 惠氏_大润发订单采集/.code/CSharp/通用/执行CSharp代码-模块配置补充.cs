//代码执行入口，请勿修改或删除
public string[] orderCategoryArr = new string[]{"DMS_Tracker", "Exception", "退单", "无新增订单",};
...
public void Run()
{
    ...
    // 20天历史订单文件路径
    dt_ModuleSetting.Columns.Add("20天历史待收订单文件", typeof(string));
    dt_ModuleSetting.Columns.Add("20天历史已收订单文件", typeof(string));
    dt_ModuleSetting.Columns.Add("今年退单文件", typeof(string));
    dt_ModuleSetting.Columns.Add("退单PDFList", typeof(object));
    ...
    dtRow_ModuleSettings["订单及详情表文件路径"] = Path.Combine(当天订单文件夹, String.Format("待收订单_{0}_{1}_{2}.xlsx", region, todayDateStr, 订单批次));
...
public void checkMailSetting(ref DataTable dt_ModuleSetting){
    ...
    foreach(string orderCat in orderCategoryArr){
        ...
        if( orderCat == "退单"){
          dtRow_ProjectSettings["退单邮件接收人"] = mailToAddress;
          dtRow_ProjectSettings["退单邮件抄送人"] = mailCcAddress;
        }
    }
}
    ...
    if(!string.IsNullOrEmpty(创单开始日期)){ // 订货开始日期 参数不为空，则使用参数
        ...
        if(lastCapturedDateDT != null && lastCapturedDateDT.Rows.Count > 0){
            ...
            if(drs.Length > 0 && (!string.IsNullOrEmpty(drs[0]["maxCreatedTime"].ToString()))){
                orderBeginDate = DateTime.Parse(drs[0]["maxCreatedTime"].ToString());
            }else{
