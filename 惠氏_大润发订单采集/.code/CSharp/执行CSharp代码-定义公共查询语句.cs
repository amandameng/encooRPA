...
public void Run()
{
    ...
    specialSkuSql = string.Format("SELECT sold_to, customer_sku_code, sku_code, comment, customer_name FROM special_products where customer_name like '{0}%'", customer_name);
    ...
    todayOrdersSql = string.Format("SELECT distinct order_date, must_arrived_by, region, customer_name, wyeth_POID,  date_format(created_time, '%Y-%m-%d') as readDate FROM {0} where customer_name='{1}' and date_format(created_time, '%Y-%m-%d')=curdate()  and order_type='订单' order by region asc, order_date", dtRow_ProjectSettings["订单数据库表名"].ToString(), customer_name);
}
//在这里编写您的函数或者类