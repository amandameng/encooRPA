...
public void Run()
{
    ...
    if(下载失败订单 != null || 抓取失败订单数据表 != null){
        ...
        if(抓取失败订单数据表!=null && 抓取失败订单数据表.Rows.Count > 0){
            ...
            异常订单邮件正文 += string.Format("抓取失败订单：{0} <br/>", string.Join("<br/>", 失败订单list));
        }
        if(下载失败订单!=null && 下载失败订单.Count > 0){
            hasDownloadExceptionOrder = true;
            下载失败订单 = getUniqList(下载失败订单);
            异常订单邮件正文 += string.Format("<br/>下载失败订单：{0} <br/>", string.Join("<br/>", 下载失败订单));
        }
        异常订单邮件标题 = string.Format("！！！雀巢【{0}】下载异常订单", curCustomerName);
    }
}
//在这里编写您的函数或者类
