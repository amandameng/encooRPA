        ...
        foreach(var orderLinkGroup in groupedOrderLinks){
            ...
            foreach(DataRow dr in orderItemRows){
                ...
                if(bulk_walfer_codes.Contains(productCode)){
                    ...
                    */
                    walmartBulkWalfer(bulk_walfer_codes, productCode, quantity_ordered, 雀巢产品编码, ref bulkWalferItemCount, dr);
                }else{
                    // 不填充EX2O ---
                    ...
                    */
                    if(remark.Contains("不填充ex2o") || remarkOption.Contains("订单类型Promotional Event为JD")){
                        continue;
                    }
                    ...
                    */
                    if(samQtyMappingDT!=null){
                        DataRow[] qtyMappingRows = samQtyMappingDT.Select(string.Format("Sam_Product_Code='{0}'", productCode));
                        if(qtyMappingRows.Length > 0){
                            ...
                            decimal nestleQty_m = quantity_ordered * Convert.ToDecimal(qtyMappingRow["Nestle_Qty"]);
                            int nestleQtyInt = Convert.ToInt32(nestleQty_m);
                            // 换算不为整数则，看产品设定是否录单
                            if((nestleQtyInt != nestleQty_m)){
                                if(Not_Integer_Still_Into_EX2O == "1"){
                                    int 山姆整层箱数 = 30;
                                    int 层数 = Convert.ToInt32(quantity_ordered/山姆整层箱数);
                                    quantity_ordered = 层数 * 山姆整层箱数;
                                }else{
                                    continue;
                                }
                            }
                    ...
                    initEtoRow(ref etoRow, dr, quantity_ordered, 雀巢产品编码, bulkWalferItemCount);
...
沃尔玛规则：
  ...
  数量≥10，口味按巧克力：牛奶：花生：  7：2：1进行拆分
*/
public void walmartBulkWalfer(List<string> bulk_walfer_codes, string productCode, int quantity_ordered, string 雀巢产品编码, ref int bulkWalferItemCount, DataRow dr){
    DataRow bulkWalferProduct = bulkWalferConfigDT.Select("customer_product_code='" + productCode + "'")[0];
    if(string.IsNullOrEmpty(bulkWalferProduct["boundry_value"].ToString())){
        return;
    }
    ...
    if(quantity_ordered < boundry_value){
        ...
        */
        ...
        for(int i=0; i< nestleCodeArr.Length; i++){
            ...
            DataRow etoRow = etoResultDT.NewRow();
            initEtoRow(ref etoRow, dr, curQuantity_ordered, nestleBulkWalferCode, bulkWalferItemCount);
...
public void initEtoRow(ref DataRow etoRow, DataRow dr, int quantity_ordered, string 雀巢产品编码, int lineNumber){
   //  DataRow etoRow = etoResultDT.NewRow();
    ...
    etoRow["PO Number"] = getPONumber(dr["order_number"].ToString());
...
public string getPONumber(string orderNumber){
    return (customer_name == "山姆-IB Water") ? $"IBU{orderNumber}" : orderNumber;
}
