...
public DataRow priceExceptionRow(DataRow dr, string 雀巢产品编码, Decimal nestle_NPS, decimal total_order_amount_after_adjustments, DataRow cleanExceptionDRow, int quantity_ordered, decimal walmartDiscountRate, bool checkSinglePrice){
    ...
    DataRow byPOItemRow =  exceptionByPODT.NewRow();
    // if checkSinglePrice为false， 也就是不检查单挑item的价差（适用拆分散威化产品的情况，只查SAP NET 总价）
        ...
        if(qtyMappingRows.Length > 0){
            ...
            if(!string.IsNullOrEmpty(qtyMappingRow["Cost_Check_Value"].ToString())){
                ...
                if(costCheckValue != cost){ // 客户网站cost跟设定的不match，则不录单
                  itemExceptionList.Add($"{byPOItemRow["沃尔玛产品编码"]}成本为{cost}，与雀巢Cost{costCheckValue}不一致");
                }
            }
        }
    }
...
public void handleExceptionRow(DataRow dr, ref DataRow cleanExceptionDRow, ref List<string> 问题订单List, DataRow[] curOrderDocLinkRows){
    ...
    */
    ...
    string 起送日 = 起送日日期.ToString("MM/dd/yyyy"); // 9
    ...
    */
    ...
    if(Math.Abs(sap_net_value - total_order_amount_after_adjustments) > 5){
        问题订单List.Add($"价差检查, SAP NET Value: {sap_net_value}, 山姆订单扣点后未税金额: {total_order_amount_after_adjustments}");
    }
        ...
        foreach(DataRow itemDR in curOrderDocLinkRows){
            ...
            if(oneToNSndDrs.Length > 0){
                foreach(DataRow oneToNProdDr in oneToNSndDrs){
                   decimal oneToNNestleNPS = Convert.ToDecimal(oneToNProdDr["Nestle_NPS"]);
...
public void setRowValueForDC(ref DataRow 分仓行, DataRow cleanExceptionDRow){
    ...
    分仓行["起送日"] = Convert.ToDateTime(cleanExceptionDRow["起送日"].ToString()).ToString("MM/dd/yyyy");;
    分仓行["MABD"] = Convert.ToDateTime(cleanExceptionDRow["MABD"].ToString()).ToString("MM/dd/yyyy");;
}