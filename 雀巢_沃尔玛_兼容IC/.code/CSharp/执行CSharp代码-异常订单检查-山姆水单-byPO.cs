...
public void handleExceptionRow(DataRow dr, ref DataRow cleanExceptionDRow, ref List<string> 问题订单List, DataRow[] curOrderDocLinkRows){
    ...
    DateTime 读单日 = Convert.ToDateTime(dr["ship_date"]);//DateTime.Now;
    string 读单当天日期 = 读单日.ToString("MM/dd/yyyy");  // 1
    ...
    */
    ...
    if(prodCode == 巴黎水产品码){
        ...
        if(gapValue > 0m || Math.Abs(每箱价差) > 0.0052525m){
            问题订单List.Add($"价差检查, SAP NET Value: {sap_net_value}, 山姆订单扣点后未税金额: {total_order_amount_after_adjustments}, 每箱价差：{每箱价差}");
        }
    }
...
public decimal getSapNetValue(DataRow dr, List<string> allitemproductCodesList, DataRow[] curOrderDocLinkRows, decimal 不含扣点){
    ...
    /*DataRow[] oneToNDrs = samWaterDeliverySettingDT.Select(string.Format("Customer_Name='{0}' and WMDC='{1}' and Customer_Product_Code in ({2})", curCustomerName, location, allitemproductCodesStr));
    
    Console.WriteLine("oneToNDrs.Length{0}, allitemproductCodesStr: {1}", oneToNDrs.Length, allitemproductCodesStr);
    if(oneToNDrs.Length > 0){ // 包含 1：n 产品码
        ...
        foreach(DataRow itemDR in curOrderDocLinkRows){
            ...
            foreach(DataRow oneToNDR in oneToNDrs){
                if(oneToNDR["Customer_Product_Code"].ToString() == customerProdCode){
