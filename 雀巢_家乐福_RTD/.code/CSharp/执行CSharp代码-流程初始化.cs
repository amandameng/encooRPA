...
public void Run()
{
    ...
    if(String.IsNullOrEmpty(流程工作目录) || !Directory.Exists(流程工作目录)){
        流程工作目录 = @"C:\RPA工作目录\雀巢_家乐福-RTD";  // TODO, 服务器上工作目录
    }
    ...
    DateTime 待预约结束日期Obj = fetchvalidDate(订货结束日期);
    订货结束日期 = 待预约结束日期Obj.ToString("yyyy-MM-dd");
    ...
    timenowStr = 订货结束日期 + timenow.ToString("-HH-mm-ss");
   // 当前结果文件夹 = Path.Combine(日结果文件夹, timenowStr);
    ...
    GlobalVariable.VariableHelper.SetVariableValue("sqlConn", sqlConn); // sqlConn
