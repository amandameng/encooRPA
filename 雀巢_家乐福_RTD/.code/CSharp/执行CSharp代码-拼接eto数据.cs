...
public void Run()
{
    ...
    orderJobHistoryDBdT = initOrderJobHistoryDT(); // orderJobHistoryDBdT
...
public DataTable initOrderJobHistoryDT(){
    DataTable resutlDT = new DataTable();
    resutlDT.Columns.Add("Customer Order Number", typeof(string));
    resutlDT.Columns.Add("Customer Order Date", typeof(string));
    resutlDT.Columns.Add("report_type", typeof(string));
    resutlDT.Columns.Add("email_sent", typeof(string));
    resutlDT.Columns.Add("email_sent_time", typeof(string));
    return resutlDT;
}

public void buildOrderJobHistory(){
    DataTable etoResultToDBDTUniq = etoResultToDBDT.DefaultView.ToTable(true, new string[]{"Customer Order Number", "Customer Order Date", });
    foreach(DataRow dr in etoResultToDBDTUniq.Rows){
        DataRow newDR = orderJobHistoryDBdT.NewRow();
        newDR["Customer Order Number"] = dr["Customer Order Number"];
        newDR["Customer Order Date"] = dr["Customer Order Date"];
        newDR["report_type"] = "EX2O";
        newDR["email_sent"] = 1;
        newDR["email_sent_time"] = DateTime.Now;
        orderJobHistoryDBdT.Rows.Add(newDR);
    }
}