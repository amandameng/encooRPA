...
public void Run()
{
    ...
    string rawSelectSql = @"SELECT oli.*, ods.order_type, ods.create_date, eo.order_expire_date as request_delivery_date, ods.logistics_warehouse,
                            ...
                            `stst`.`Request_Delivery_Date` as ststRDD, `stst`.daysSpent as 在途天数,
                            ...
                            left join material_master_data npmd on npmd.`Customer_Material_No` = oli.product_code and npmd.`Customer_Name` = ods.customer_name 
                            and replace(npmd.`Nestle_Plant_No`, ' ', '') = replace(`stst`.`Nestle_Plant_No`, ' ', '') and npmd.Ship_to_Code = stst.Ship_to_Code
