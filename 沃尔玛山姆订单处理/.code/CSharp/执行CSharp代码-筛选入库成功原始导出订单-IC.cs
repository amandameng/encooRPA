...
public void Run()
{
    ...
    validOrdersDT =excludeFailedOrder(实际增量订单数据表, 抓取失败订单列表);
    Console.WriteLine($"validOrdersDT: {validOrdersDT.Rows.Count.ToString()}");
...
public DataTable excludeFailedOrder(DataTable 实际增量订单数据表, List<string> 抓取失败订单列表){
