...
public void buildEdiData(){
    ...
    foreach (var itemGroup in ex2oGroupByOrders)
    {
       ...
       // bool ifOrderIntoEDI = true;
        ...
        // 如果数据完整则检查Item
        List<DataRow> orderAndItemRows = new List<DataRow>{};
        if(orderExceptionFieldsList.Count == 0){
            orderAndItemRows.Add(newEdiOrderRow);  // Add Row for Order
            foreach(DataRow dRow in groupedDrs){
                DataRow newEdiOrderItemRow = buildEdiOrderItemRow(dRow, ref orderExceptionFieldsList, ref orderDataInvalidFieldsList);
                if(!is_edi_production) updateOrigEX2OSapMaterial(newEdiOrderItemRow, orderNumber, dRow["SAP_Material"].ToString()); // 更新原始订单为测试物料号
                if(orderExceptionFieldsList.Count == 0){
                    orderAndItemRows.Add(newEdiOrderItemRow);
                }
            }
        }
        if(orderExceptionFieldsList.Count == 0 && orderDataInvalidFieldsList.Count == 0){  // 无异常信息
            foreach(DataRow dr in orderAndItemRows){
                 ediDT.Rows.Add(dr);
            }
        }else{ // 记录问题订单号及异常信息
...
public DataRow buildEdiOrderRow(DataRow orderRow, ref List<string> orderExceptionFieldsList, ref List<string> orderDataInvalidFieldsList){
    ...
    // 检查必填项是否为空
    string[] mandatoryFields = new string[]{ "Sales_Order_Type",  "Sales_Org", "Distribution_channel", "Sold_to", "Ship_to", "PO_Number", "Reqd_Del_Date"};
    checkFields(orderRow, mandatoryFields, ref orderExceptionFieldsList, ref orderDataInvalidFieldsList);
    if(orderExceptionFieldsList.Count > 0 || orderDataInvalidFieldsList.Count > 0){
        return ediRow;
    }
    ...
    if(is_edi_production){
        ...
        ediRow["E"] = soldToCode;  // mandatory
        ediRow["G"] = shipToCode;  // mandatory
    }
    ediRow["F"] = orderRow["Sold_to_Name"];
    ediRow["H"] = orderRow["Ship_to_Name"];
    ediRow["I"] = orderRow["PO_Number"];  // mandatory
    ediRow["J"] = Convert.ToDateTime(orderRow["Reqd_Del_Date"]).ToString("yyyyMMdd"); // mandatory
...
public DataRow buildEdiOrderItemRow(DataRow orderRow, ref List<string> orderExceptionFieldsList, ref List<string> orderDataInvalidFieldsList){
    DataRow ediRow = ediDT.NewRow();
    
     // 检查必填项是否为空
    string[] mandatoryFields = new string[]{ "SAP_Material",  "Qty", "UoM"};
    ...
    if(is_edi_production){
        ...
        ediRow["B"] = materialCode;  // mandatory
    }
