...
public void Run()
{
            ...
            foreach(string pdfItem in pdfArray){
                ...
                if(pdfItem.IndexOf("备注") > -1){
                    ...
                    if(tableDataIndex == 19) {
                        ...
                        orderItemRow["Quantity"] = Convert.ToInt32(订购倍数);
        ...
        #region 合规性检查
            ...
            if(地址.IndexOf("无锡") > -1 || 地址.IndexOf("杭州") > -1){
                ...
                if(Convert.ToDateTime(预定交货日).Month > 10 || Convert.ToDateTime(预定交货日).Month < 3){  //淡季
                    // 订单起送量 = orderItemTable.AsEnumerable().Cast<DataRow>().Sum(s => Convert.ToInt32(s["Quantity"].ToString()));
                    if(总订购倍数 < 10){
                        起送量检查 = false;
                        错误信息 = "淡季市内起送量不足10cs";
            ...
            if(地址.IndexOf("无锡") > -1 || 地址.IndexOf("杭州") > -1){
                if(Convert.ToDateTime(timeNowDateStr).AddDays(2) > Convert.ToDateTime(预定交货日)){
                    交货日检查 = false;
                ...
                if(Convert.ToDateTime(timeNowDateStr).AddDays(1) > Convert.ToDateTime(预定交货日)){
                    交货日检查 = false;
        ...
        #region 写入Clean 或者 Exception 数据表
            if(!string.IsNullOrEmpty(错误信息)){ //写入Exception表  
                     ...
                     if ((excelItem["PONumber"]).ToString() == "IBU" + 订单编号)
                        {
                            ...
                            exceptionRow["客户要求送货日"] = Convert.ToDateTime(预定交货日).ToString("yyyy/MM/dd");
                            ...
                            //}
                                 ...
                                 exceptionRow["问题分类"] = "无法mapping雀巢产品";
                            }                   
                ...
                cleanRow["订单数量"] = 总订购倍数;
                cleanRow["要求送货日"] = Convert.ToDateTime(预定交货日).ToString("yyyy/MM/dd");
